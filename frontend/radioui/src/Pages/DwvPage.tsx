...
const DwvPage: React.FC = () => {
  ...
  const [selectedScansDone, setSelectedScansDone] = useState(patientInfo?.patientScansDone);

  const handleChange = (event) => {
    const newScansDone = event.target.value;
    setSelectedScansDone(newScansDone);

    // // Assume you have a function to handle the update
    // onUpdateRemarks(patientInfo.doctorId, patientInfo.patientId, patientInfo.remarks, newScansDone);
  };
        ...
        <Paper elevation={3} style={{ padding: '20px', marginTop: 85, height: '80vh', overflowY: 'auto' }}>
          ...
          {loading ? (
            ...
            <div>
              ...
              <Typography>Scans Done: 
        <Select value={selectedScansDone} onChange={handleChange}>
